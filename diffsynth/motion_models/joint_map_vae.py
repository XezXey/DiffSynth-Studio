...
from diffsynth.models.wan_video_vae import Decoder3d, WanVideoVAE, count_conv3d, CausalConv3d
...
class Head(th.nn.Module):
    def __init__(self, dim: int, out_dim: int, patch_size: Tuple[int, int, int]):
        ...
        # self.norm = th.nn.LayerNorm(dim, eps=eps, elementwise_affine=False)
        self.head = th.nn.Linear(dim, out_dim * math.prod(patch_size))
        # self.modulation = th.nn.Parameter(th.randn(1, 2, dim) / dim**0.5)
...
class JointHeatMapMotionVAEDecoder(th.nn.Module):
    ...
    def forward(self, pipe, dit_features, grid_size):
        ...
        final_joint_map = []
        #TODO: why shape has 1, 23, 4, 16, 8, 8 instead of 1, 23, 16, 16, 8, 8?
