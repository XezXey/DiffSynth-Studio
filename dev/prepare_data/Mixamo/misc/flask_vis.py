        ...
        <div class="sequence" id="seq-{{ seq.id }}">
            ...
            <div class="image-wrapper">
                ...
                <img id="overlay-{{ seq.id }}" class="overlay-image"
                     src="/proj/{{ seq.name }}/{{ seq.first_index }}">
            </div>
            <div class="controls">
...
function setupSequence(seq){
    ...
    const updateImage=()=>{
        ...
        overlay.src="/proj/"+encodeURIComponent(state.name)+"/"+f;
...
@app.route("/proj/<path:seq_name>/<int:frame_number>")
def serve_proj(seq_name, frame_number):
    """
    Serve proj%04d.png from DATA_ROOT/seq_name (for overlay).
    If proj file is missing, this will 404; overlay just won't show.
    """
    if DATA_ROOT is None:
        abort(404)
    ...
    filename = f"proj{frame_number:04d}.png"
    file_path = seq_dir / filename
    if not file_path.is_file():
        abort(404)
