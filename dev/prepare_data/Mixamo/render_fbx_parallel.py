...
def render_animation(
    ...
    if not legacy_mode:
        # Cycles engine - ray-traced, works headless
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_gpu', action='store_true', help='Enable GPU rendering with Cycles')
    ...
    print(f"[#] Render engine: cycles", flush=True)
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=n_workers) as pool:
                ...
                with Progress(
                    ...
                    task_id = progress.add_task(
                        f"Rendering {args.n_cam} cams × {n_frame_w} frame workers",
                        total=len(futures),
                    )
                    for fut in concurrent.futures.as_completed(futures):
                        (ci, fwi), rc = fut.result()
                        completed_count += 1
                        tag = f"cam {ci}" + (f" / fw {fwi}" if fwi is not None else "")
                        status = "[green]OK[/green]" if rc == 0 else f"[red]FAILED rc={rc}[/red]"
                        progress.console.log(f"[{completed_count}/{total_jobs}] {tag} → {status}")
